 ...
 */
Auth::routes([
    'verify' => !config('auth.active_directory.enabled', false),
    'reset' => !config('auth.active_directory.enabled', false),
    'confirm' => !config('auth.active_directory.enabled', false),
]);
...
Route::group(['middleware' => ['auth', 'verified']], function () {
    ...
    Route::get('modules/details/{module}', 'Admin\ModuleController@getDetails');
    ...
    /* Ruta para obtener los parámetros generales activos del modulo payroll */
    Route::get('parameters/getParameters', 'ParameterController@getParameters')->name('parameters.parameters');
    ...
    Route::get('get-genders', 'GenderController@getGenders')->name('get-genders');
    ...
    Route::get('fiscal-years/list', 'FiscalYearController@getAll')->name('all.fiscal-years');
    ...
    Route::get(
        'all-receivers',
        'ReceiverController@getAllReceivers'
    )->name('get.all-receivers');

    Route::resource('headquarters', 'HeadquarterController', ['except' => ['create', 'show', 'edit']]);
    Route::get('get-headquarters', 'HeadquarterController@getHeadquarters')->name('get-headquarters-list');

    Route::group([
        'middleware' => ['web', 'auth', 'verified'],
        'prefix' => 'close-fiscal-year',
        'as' => 'close-fiscal-year.'
    ], function () {
        /* Ruta de recursos para la gestión de configuración de cierre de ejercicio económico */
        Route::resource('settings', 'CloseFiscalYearSettingController', ['only' => ['index', 'store']]);
        Route::post(
            'settings/add-account',
            'CloseFiscalYearSettingController@storeAccount'
        )->name('settings.add-account');
        Route::get(
            'settings/get-account',
            'CloseFiscalYearSettingController@getAccountingAccount'
        )->name('settings.get-account');
        Route::get('entries/create', 'CloseFiscalYearController@createEntries')->name('entries.create');
        Route::resource('registers', 'CloseFiscalYearController', ['only' => ['index', 'store', 'destroy']]);
        Route::get('registers/vue-list', 'CloseFiscalYearController@vueList')->name('registers.vue-list');
        Route::post('registers/approve/{id}', 'CloseFiscalYearController@approve')->name('registers.approve');
        Route::post('registers/search', 'CloseFiscalYearController@searchCloseFiscalYear')->name('registers.search');
    });
});
