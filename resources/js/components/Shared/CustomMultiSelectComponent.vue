    ...
    <div class="custom-multiselect" style="display: grid;">
        <div class="btn-group" style="background-color: white; color: darkgray; white-space: nowrap;">
                ...
                <div class="multiselect__tags" style="display: flex; flex-wrap: wrap;">
                    <div class="multiselect__tags-wrap" style="inline-grid"
                        ...
                        <span class="multiselect__tag" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                            {{ selection[track_by] }}
                            <span class="badge badge-light" v-if="selection.count > 1">
                                {{ selection.count }}
                            </span>
                            <i tabindex="1" class="multiselect__tag-icon" @click="removeSelection(index, $event)"></i>
                        </span>
                    </div>
                    <input
                        ref="search"
            ...
            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left"
                style="max-height: 15rem; font-size: 16px; overflow-y: auto; overflow-x: auto; max-width: 100%;">
  ...
  export default {
    ...
    methods: {
        addSelection(option, event) {
            const existingSelection = this.selections.find(sel => sel.id === option.id);
        ...
        filterOptions (options, search, value) {
            return options.filter(option => this.includes(option[value], search))
        },
    },
    mounted() {
        const vm = this;
        ...
        const elementos = document.querySelectorAll('.custom-multiselect .dropdown-menu');
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                const isActive = mutation.target.classList.contains('show');
                const dropdownInput = mutation.target.previousElementSibling.lastElementChild.previousElementSibling.lastElementChild;
                if (isActive) {
    ...
    computed: {
        filteredOptions () {
            ...
            options = this.filterOptions(options, normalizedSearch, this.track_by)
