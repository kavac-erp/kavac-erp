<template>
	<section class="row" id="draggableTable">
		...
		<div class="col-md-12" style="overflow-x: auto;">
			<table ref="draggableTable" class="table table-hover table-striped draggable-table table-responsive" style="display: table;">
				<thead>
					<tr>
						<th v-for="(column, index) in columns"
							:key="index"
							:class="{'draggable-column': true,
								'highlighted-column': (index !== dragIndex && dragIndex !== null)}"
				...
				<tbody>
					<tr v-for="(row, rIndex) in visibleRows"
						:key="rIndex">
						<td v-for="(column, index) in columns"
							...
							<span v-else-if="column.type == 'input' && column.group">
								<input type="text" class="form-control input-sm" data-toggle="tooltip"
									title="Indique el valor del campo" v-model="inputValues[column.name + '-' + row.staff_id]"
									v-input-mask data-inputmask="'alias': 'integer', 'allowMinus': 'false'" maxlength="3" :disabled="column.disabled">
							</span>
							<span v-else-if="column.type == 'input_text'">
								<input type="text" class="form-control input-sm" data-toggle="tooltip"
									title="Indique el valor del campo" v-model="inputValues[column.name + '-' + row.staff_id]"
									v-is-text :disabled="column.disabled">
							</span>
							<span v-else-if="column.type == 'custom'">
								<div class="d-flex text-center" v-for="(component, i) in custom_components" :key="i">
									<div class="col-6" v-if="column.name == component['column']">
										<i class="fa fa-plus-circle cursor-pointer" v-if="!inputValues[column.name + '-' + row.staff_id]"
											@click="setCustomComponent(component['ref'], row.staff_id, component['modalId'])">
										</i>
										<i class="fa fa-eye cursor-pointer" v-else
											@click="setCustomComponent(component['ref'], row.staff_id, component['modalId'], inputValues[column.name + '-' + row.staff_id], column.field)">
										</i>
									</div>
								</div>
							</span>
							<span v-else-if="column.type == 'subtotal'"
									:class="(column.max && calculate[column.group+'-'+row.staff_id] > column.max)
										? 'form-control text-center align-middle info-danger'
									: 'form-control text-center align-middle'">
								<input type="hidden" class="form-control input-sm" data-toggle="tooltip"
									title="Indique el valor del campo"
									:v-model="inputValues['subtotal - '+columns[index-1].group+'-'+row.staff_id] = calculate[columns[index-1].group+'-'+row.staff_id]"
									disabled>
								{{ calculate[columns[index-1].group+'-'+row.staff_id] }}
							</span>
							<span v-else-if="column.type == 'total'">
								<input type="hidden" class="form-control input-sm" data-toggle="tooltip"
									title="Indique el valor del campo"
									:v-model="inputValues['total-'+row.staff_id] = calculate['total-'+row.staff_id]"
									disabled>
	...
	export default {
		...
		props: {
			...
			options: {
				...
				default: function() {
					return {
						'draggableBy': this.columns.filter(function (option) {
							return true === (option.isDraggable ?? true);
						}).map(function (option) {
							return option.name;
						}),
						'inputBy': this.columns.filter(function (option) {
			...
			search(res) {
				...
				if (res == '') {
					...
					this.tmpData = this.data.filter((item) => {
						...
						return nombre.includes(res) || ficha.includes(res) || n.includes(res)
					})
				}
		...
		methods: {
			...
			handleDragDrop(event, index) {
				...
				if (sourceIndex !== index && vm.columns[index]['isDraggable'] === true) {
			...
			const intervalId = setInterval(() => {
				...
				if (ths.length > 0) {
					ths.forEach((th, index) => {
						...
						th.addEventListener('dragstart', (e) => vm.handleDragStart(e, index));
						th.addEventListener('dragenter', (e) => vm.handleDragEnter(e, index));
						th.addEventListener('dragleave', (e) => vm.handleDragLeave(e, index));
						th.addEventListener('dragover', (e) => e.preventDefault());
						th.addEventListener('drop', (e) => vm.handleDragDrop(e, index));
			...
			calculate: function () {
				...
				vm.data.forEach((row, rIndex,) => {
					vm.columns.forEach(column => {
						if (column.group) {
							groups[column.group+'-'+row.staff_id] = ((vm.inputValues[column.name + '-' + row.staff_id]) ? parseFloat(vm.inputValues[column.name + '-' + row.staff_id]) : 0) + ((groups[column.group+'-'+row.staff_id]) ? parseFloat(groups[column.group+'-'+row.staff_id]) : 0);
							groups['total-'+row.staff_id] = ((groups['total-'+row.staff_id]) ? parseFloat(groups['total-'+row.staff_id]) : 0) + ((vm.inputValues[column.name + '-' + row.staff_id]) ? parseFloat(vm.inputValues[column.name + '-' + row.staff_id]) : 0);
						}
					})
				});
				return groups;
