    ...
    <body>
            ...
            @else
                <div id="app">
                    ...
                    <div class="content-wrapper">
                        @php
                            $fiscalYear = \App\Models\FiscalYear::where(['active' => true, 'closed' => false])->latest()->first();
                        @endphp
                        @if(isset($fiscalYear) && $fiscalYear->year < date('Y'))
                            <div style="z-index: 1001;" class="position-fixed col-12 alert alert-danger" role="alert">
                                <div class="d-flex justify-content-center align-items-center" style="gap: 5px">
                                    <i class="now-ui-icons objects_support-17"></i>
                                    <strong>¡ATENCIÓN!</strong>
                                    SE DEBE REALIZAR EL CIERRE DE EJERCICIO DEL AÑO FISCAL {{ $fiscalYear->year }}.
                                </div>
                            </div>
                            <br><br>
                        @endif
        ...
        @auth
            <script>
                    ...
                    if (parent_id) {
                        axios.get(
                            ...
                            if (response.data.result) {
                                ...
                                $.each(response.data.records, function(index, record) {
                                     target_element.append(
                                        `<option value="${record['payrollPosition'].id}">${record['payrollPosition'].name}</option>`
                                    );
                 ...
                 * @param  {string}  target_model   Modelo en el cual se va a realizar la consulta
                 * @param  {string}  module_name    Nombre del módulo que ejecuta la acción
                 * @param  {string}  relation    Nombre de relacion particular que nesecite traer en la consulta
                 * @param  {array}   disables    array de campos relacionados al objetivo que deben limpiar y desabilitarse en caso de cambio del campo objetivo
                 */
                function updateSelectActive(parent_element, target_element, target_model, module_name, relation, disables) {
                    var module_name = (typeof(module_name) !== "undefined")?'/' + module_name:'';
                ...
                /**
                 * Imprime cédula, nombre y apellido en el select de Presupuesto > Proyectos y
                 * Presupuesto > Acciones centralizadas, a partir de otro además de que trae datos con atributo activo
                 *
                 ...
                 * @param  {string}  module_name    Nombre del módulo que ejecuta la acción
                 * @param  {string}  relation       Nombre de relacion particular que nesecite traer en la consulta
                 * @param  {array}   disables       Arreglo de campos relacionados al objetivo que deben limpiar y
                 *                                  desabilitarse en caso de cambio del campo objetivo
                 */
                function updateStaffSelect(parent_element, target_element, target_model, module_name, relation, disables) {
                    var module_name = (typeof(module_name) !== "undefined")?'/' + module_name:'';
                    var passthru = relation;
                    var relation = (typeof(relation) !== "undefined") ? '/' + relation : '';
                    var parent_id = parent_element.val();
                            ...
                            if (response.data.result){
                                ...
                                $.each(response.data.records, function(index, record) {
                                    if ((typeof(passthru) !== "undefined")){
                                        if (passthru == "payrollStaff"){
                                            target_element . append(
                                                `<option value="${record.payroll_staff.id}">
                                                    ${record.payroll_staff.id_number} -
                                                    ${record.payroll_staff.first_name}
                                                    ${record.payroll_staff.last_name}
                                                </option>`
                                            );
                                        } else {
                                            target_element . append(
                                                `<option value="${record[relation].id}">
                                                    ${record[relation].id_number} -
                                                    ${record[relation].first_name}
                                                    ${record[relation].last_name}
                                                </option>`
                                            );
                                        }
                                    } else {
