...
<nav class="navbar navbar-expand-lg bg-info fixed-top">
    ...
    <div
        class="collapse navbar-collapse" id="app-navbar-info" data-nav-image="{{ asset('images/blurred-image.jpg') }}"
    >
            ...
            @if ($institution)
                <div class="navbar-brand">
                    @if ($institution->logo()->exists() && file_exists(base_path($institution->logo->url)))
                        <img
                            src="{{ asset($institution->logo->url, Request::secure()) }}" alt="{{ __('logo') }}" 
                            class="img-fluid"
                        >
                    @endif
        ...
        <ul class="navbar-nav">
            @if (Auth::user()->hasVerifiedEmail())
                ...
                @if ($notify)
                    <notifications
                        ...
                        list-notifications-url="{!! route('notifications.list') !!}"
                    />
                    </notifications>
                @endif
                ...
                @if ($chat)
                    <li class="nav-item">
                        <a
                            class="nav-link btn btn-sm btn-info"
                            href="javascript:void(0)" title="{{ __('Chat') }}"
                            data-toggle="tooltip">
                ...
                @if ($support)
                    <li class="nav-item">
                        <a class="nav-link btn btn-sm btn-info"
                            ...
                            title="{{ __('Contacte con soporte técnico') }}"
                            data-toggle="tooltip">
            ...
            <li class="nav-item dropdown">
                <a href="javascript:void(0)"
                    class="nav-link dropdown-toggle btn btn-sm btn-info"
                    id="list_options_user" data-toggle="dropdown"
                    aria-expanded="false"
