...
<nav class="navbar navbar-expand-lg bg-info fixed-top">
    <div class="container-left">
        ...
        <div class="float-right">
            <div class="menu-collapse" data-toggle="tooltip" data-placement="right"
                title="{{ __('Minimizar panel de menú') }}">
    ...
    <div class="collapse navbar-collapse" id="app-navbar-info" data-nav-image="{{ asset('images/blurred-image.jpg') }}">
        <div class="navbar-translate">
            @php
                if (isset(auth()->user()->profile)) {
                    if (isset(auth()->user()->profile->institution_id)) {
                        $institution = App\Models\Institution::where([
        ...
        <ul class="navbar-nav">
            @if (Auth::user()->hasVerifiedEmail())
                ...
                @if ($notify)
                    <notifications :unreads="{{ auth()->user()->unreadNotifications }}"
                        :user-id="{!! auth()->user()->id !!}" list-notifications-url="{!! route('notifications.list') !!}" />
                    </notifications>
                @endif
                ...
                @endif
                @if (Auth::user()->hasRole('admin') && config('backup.backup.enabled', true))
                    <li class="nav-item">
                        <a class="nav-link btn btn-sm btn-info" href="{{ route('backup.index') }}"
                            title="{{ __('Respaldos de Base de Datos') }}" data-toggle="tooltip">
            ...
            <li class="nav-item dropdown">
                <a href="javascript:void(0)" class="nav-link dropdown-toggle btn btn-sm btn-info" id="list_options_user"
                    data-toggle="dropdown" aria-expanded="false" title="{{ __('Mi configuración y datos') }}">
