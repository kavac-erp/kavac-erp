...
Vue.component('register-order-list', () => import(
    /* webpackChunkName: "register-clients" */
    './components/order/SaleOrderComponentList.vue')
);
...
Vue.component('payment-registered-list', () => import(
    ...
    './components/payment/SalePaymentsComponentList.vue')
);
	...
	methods: {
		...
		getSaleSettingProductType() {
			...
			axios.get('/sale/get-setting-product-type').then(response => {
				vm.product_types = response.data;
			});
		},
		...
		/**
		 * Obtiene un arreglo con los estados de los pedidos
		 *
		 * @author Juan Vizcarrondo <jvizcarrondo@cenditel.gob.ve> | <juanvizcarrondo@gmail.com>
		 */
		getOrderStatus() {
			const vm = this;
			vm.order_status_list = [];
			axios.get('/sale/get-order-status').then(response => {
				vm.order_status_list = response.data;
			});
		},

		/**
		 * Obtiene un arreglo con la lista con los clientes con pedidos
		 *
		 * @author Juan Vizcarrondo <jvizcarrondo@cenditel.gob.ve> | <juanvizcarrondo@gmail.com>
		 */
		getOrderWithClients() {
			const vm = this;
			vm.order_with_clients = [];
			axios.get('/sale/get-order-with-clients').then(response => {
				vm.order_with_clients = response.data;
			});
		},

		/**
		 * Obtiene un arreglo con la fecha minima y maxima de pedidos
		 *
		 * @author Juan Vizcarrondo <jvizcarrondo@cenditel.gob.ve> | <juanvizcarrondo@gmail.com>
		 */
		getSaleOrderYear() {
			const vm = this;
			vm.order_years = [];
			axios.get('/sale/get-order-years').then(response => {
				vm.order_years = response.data;
			});
		},

		/**
		 * Obtiene un arreglo con la lista del rango de fechas de pedidos
		 *
		 * @author Juan Vizcarrondo <jvizcarrondo@cenditel.gob.ve> | <juanvizcarrondo@gmail.com>
		 */
		getSaleOrderRangeDates() {
			const vm = this;
			vm.order_range_dates = [];
			axios.get('/sale/get-quote-range-dates').then(response => {
				vm.order_range_dates = response.data;
			});
		},
