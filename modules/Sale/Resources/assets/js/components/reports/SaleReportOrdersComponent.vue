      ...
      <div class="row align-items-center">
        ...
        <div class="col-md-6 mt-2">
          <div class="form-check">
            ...
            <label class="form-check-label" for="CheckStatus">Estado del Pedido</label>
          </div>
        </div>
        ...
        <div class="col-md-6 mt-2" v-if="record.check_status">
          <div class="form-group is-required">
            <select2 :options="order_status_list" v-model="record.status"></select2>
          </div>
        </div>
        <div class="col-md-6 mt-2">
          <div class="form-check">
        ...
        <div class="col-md-6 mt-2" v-if="record.check_client">
          <div class="form-group is-required">
            <v-multiselect :options="order_with_clients" track_by="id"
      ...
      <v-client-table :columns="columns" :data="records" :options="table_options">
        ...
        <div slot="products" slot-scope="props">
          <div v-if="props.row.products">
            <ul v-for="(product, index) in props.row.products" :key="index">
              <li>{{ product.name }}</li>
            </ul>
          </div>
          <div v-else>
...
export default {
  data() {
    return {
      reportUrl : 'sale/reports/orders/pdf',
      record: {
        filterDate : '',
        dateIni : '',
        dateEnd : '',
  ...
  methods: {
    ...
    */
    searchRecords(){
      ...
      vm.records = [];
      vm.recordsToReport = [];
      axios.post('/sale/reports/orders/data', this.record).then(response => {
  ...
  mounted(){
    const vm = this;
    vm.reset();
    vm.getOrderWithClients();
    vm.getOrderStatus();
    vm.getSaleOrderYear();
    vm.getSaleOrderRangeDates();
  },
};
</script>
