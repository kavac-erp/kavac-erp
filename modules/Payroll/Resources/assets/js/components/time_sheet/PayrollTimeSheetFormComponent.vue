<template>
    <section id="PayrollEmploymentForm">
        ...
        <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group is-required">
                        ...
                        <input type="date" id="from_date"
                                ...
                                v-model="record.from_date" data-toggle="tooltip"
                                @input="loadDataCompletedPeriods"
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group is-required">
                        ...
                        <input type="date" id="to_date" placeholder="Hasta"
                                class="form-control no-restrict input-sm"
                                @input="loadDataCompletedPeriods"
                                v-model="record.to_date" data-toggle="tooltip"
                                title="Indique el periodo (requerido)">
                    </div>
                    <!-- ./Hasta -->
                </div>
                <div class="col-md-6">
                    ...
                    <div class="form-group is-required">
                        ...
                        <select2 :options="payroll_supervised_groups"
                                    ...
                                    @input="getSupervisedGroupData(); setTimeSheetData(); loadDataCompletedPeriods();">
                        </select2>
                    </div>
                    <!-- ./Código -->
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group is-required">
                        ...
                        <select2 :options="payroll_time_sheet_parameters"
                                    ...
                                    @input="setTimeSheetColumns(); loadDataCompletedPeriods()">
                        </select2>
                    </div>
                    <!-- ./Parámetros de la hoja de tiempo -->
                </div>
            ...
            <v-draggable-table ref="draggableTable" :columns="draggableColumns" :data="draggableData"></v-draggable-table>
        </div>
        <!-- Final card-body -->
        ...
        <div class="card-footer text-right" id="helpParamButtons">
            ...
            <button
                type="button"
                @click="createRecord('payroll/time-sheet')"
                data-toggle="tooltip"
        ...
        methods: {
            ...
            setTimeSheetColumns() {
                ...
                if (vm.record.payroll_time_sheet_parameter_id) {
                    if (!vm.record.id || vm.record.time_sheet_columns == null) {
                        ...
                        let params = vm.payroll_time_sheet_parameters.find(function ($param) {
                            return vm.record.payroll_time_sheet_parameter_id == $param['id'];
                        })
    
                        Object.values(params.parameters).forEach((param, index, array) => {
                            let group = '';
                            let groupMax = '';
                            param.forEach(p => {
                                if ('' === group) {
                ...
                if (vm.record.id) {
                    Vue.set(vm.$refs.draggableTable, 'inputValues', vm.record.time_sheet_data);
                }
            },
            ...
            async createRecord(url, list = true, reset = true) {
                ...
                if (vm.record.id) {
                    ...
                    if (vm.$refs.draggableTable && typeof(vm.$refs.draggableTable) != 'undefined') {
                        let columns = [];
                        ...
                        vm.$refs.draggableTable.columns.forEach((column, index) => {
                            ...
                            if (updatedColumn.name.includes('Subtotal')) {
                                updatedColumn.name = 'subtotal' + ' - ' + updatedColumn.group;
                            }
            ...
            async loadForm(id) {
                ...
                let recordEdit = await axios.get(`${window.app_url}/payroll/time-sheet/vue-info/${id}`).then(response => {
            ...
            exportTimeSheetData() {
                ...
                let data = {
                    columns: vm.$refs.draggableTable.columns,
                    data: vm.$refs.draggableTable.data,
                    inputValues: vm.$refs.draggableTable.inputValues
                };

                axios.post(`${window.app_url}/payroll/time-sheet/export`, { params: data }, { responseType: 'blob' }).then(response => {
            ...
            importTimeSheetData() {
                ...
                axios.post(`${window.app_url}/payroll/time-sheet/import`, formData, {
                    headers: {
                    ...
                    columns.forEach(column => {
                        ...
                        columnName = columnName.replace(/ - /g, '_').replace(/ /g, '_');
                    ...
                    for (let i = 0; i < formattedColumns.length; i++) {
                        ...
                        let valor = response.data.find(item => item.n === n && item[column_name] !== null);
                        ...
                        if (
                            ...
                            Vue.set(vm.$refs.draggableTable.inputValues, col_name, parseInt(valor[column_name]));
                        }
                    }

                    vm.showMessage(
                        'custom', '!Éxito!', 'success', 'screen-ok', 'Se ha realizado el proceso de importación exitosamente'
                    );
                }).catch(error => {
                    console.error(error);
                });
                    ...
                    if (vm.$refs.draggableTable && typeof(vm.$refs.draggableTable) != 'undefined') {
                        await axios.get(
                            ...
                            { params: {
                                ...
                                payroll_supervised_group_id: vm.record.payroll_supervised_group_id,
                                payroll_time_sheet_parameter_id: vm.record.payroll_time_sheet_parameter_id,
                            } })
                        .then(response => {
                            if (typeof(response.data.result) !== "undefined" && (Object.keys(response.data.result).length > 0)) {
                                bootbox.confirm({
                                    ...
                                    callback: (result) => {
                                        if (result) {
                                            ...
                                            $('.preloader').show();
                                            vm.draggableColumns.forEach(column => {
                                                if (column.type == 'input') {
                                                    Vue.set(column, 'disabled', (response.data.confirmed ? response.data.confirmed : []).includes(column.name));
                                                }
                                            })
                                            for (let key in response.data.result) {
                                                const lastIndex = key.lastIndexOf("-");
                                                const index = key.slice(0, lastIndex).trim();
                                                const staffId = key.slice(lastIndex + 1).trim();
                                                const foundColumn = vm.draggableColumns.some(obj => obj.name == index);
                                                const foundStaff = vm.draggableData.some(obj => obj.staff_id == staffId);

                                                if (foundColumn && foundStaff) {
                                                    Vue.set(vm.$refs.draggableTable.inputValues, key, response.data.result[key]);
                                                }
                                            }
