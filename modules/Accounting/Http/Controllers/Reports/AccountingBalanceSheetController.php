...
class AccountingBalanceSheetController extends Controller
{
    ...
    ")
                ...
                ->where(function ($query) use ($formDate, $institution_id, $currency_id, $is_admin) {
                    if ($institution_id) {
                        if (
                        ...
                        if ($is_admin) {
                            if ($query->where('accounting_entries.from_date', '<', $formDate)
                                ->where('accounting_entries.approved', true)) {
                                $query->where('accounting_entries.from_date', '<', $formDate)
                                    ->where('currency_id', $currency_id)
                                    ->where('accounting_entries.approved', true);
    ...
    /**
     * [getTotalResults realiza el calculo de las cuentas de acuerdo a como suman]
     * @author fjescala <fjescala@gmail.com>
     * @param  array   $convertions   [lista de tipos cambios para la moneda]
     * @param  integer $ini     [identificador del tipo de cuenta]
     * @param  integer $debit     [valor de la cuenta debe]
