	...
	{{-- Se recorren las cuentas de Inicio Nivel 1 --}}
		@php
                    if(array_key_exists("5.0.0.00.00.00.000",$records))
						$result_of_the_excersice += ($records["5.0.0.00.00.00.000"]['beginningBalance']+$records["5.0.0.00.00.00.000"]['balance']);
                    if(array_key_exists("6.0.0.00.00.00.000",$records))
						$result_of_the_excersice -= ($records["6.0.0.00.00.00.000"]['beginningBalance']+$records["6.0.0.00.00.00.000"]['balance']);
	@endphp
	...
	@endphp
	   ...
	   @if($next )
		...
		<tr style="background-color: #BDBDBD;">
			...
			<td style="font-size:9rem; {{ $styles }}" width="32%">&nbsp;{{ $parent['denomination'] }}</td>
			<td style="font-size:9rem; {{ $styles }}" align="right" width="15%">{{ number_format($parent['beginningBalance'], (int)$currency->decimal_places, ',', '.') }}</td>
			<td style="font-size:9rem; {{ $styles }}" align="right" width="15%">{{ number_format($parent['balance'], (int)$currency->decimal_places, ',', '.') }}</td>
