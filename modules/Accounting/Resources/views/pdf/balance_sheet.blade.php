...
<h3 align="center" style="font-size: 9rem;">BALANCE GENERAL AL {{ explode('-',$endDate)[2].'-'.explode('-',$endDate)[1].'-'.explode('-',$endDate)[0] }}</h3>
<h5 style="font-size: 8rem;">EXPRESADO EN {{ $currency->symbol }}</h5>
		...
		@foreach($records as $key => $parent)
			...
			@php
				...
				@if ($p['parent'] === null)
					<tr style="background-color: #BDBDBD;">
						...
						<td style="font-size:9rem; {{ $styles }}" width="60%">&nbsp;{{ $p['denomination'] }}</td>
						<td style="font-size:9rem; {{ $styles }}" align="right" width="24%"></td>
					</tr>
				@else
					<tr>
						<td style="font-size:9rem; {{ $styles }}" width="16%">&nbsp;{{ $p['code'] }}</td>
						<td style="font-size:9rem; {{ $styles }}" width="32%">&nbsp;{{ $p['denomination'] }}</td>
						<td style="font-size:9rem; {{ $styles }}" align="right" width="15%">&nbsp;{{  number_format($p['lastMonthBalance'], (int)$currency->decimal_places, ',', '.') }}</td>
						<td style="font-size:9rem; {{ $styles }}" align="right" width="15%">&nbsp;{{ number_format($p['balance'], (int)$currency->decimal_places, ',', '.') }}</td>
						<td style="font-size:9rem; {{ $styles }}" align="right" width="20%">&nbsp;{{ number_format($p['balance']+$p['lastMonthBalance'], (int)$currency->decimal_places, ',', '.') }}</td>
					</tr>
				@endif
			@endforeach
			<tr style="background-color: #BDBDBD;">
			...
			@if($key == 3)
				...
				<tr style="background-color: #BDBDBD;">
					...
					<td style="font-size: 9rem; font-weight: bold;" align="right">{{ number_format($totPasivePatrimonialLast, (int)$currency->decimal_places, ',', '.') }}</td>
				</tr>
				<tr>
					<td style="font-size: 9rem;"></td>
					<td style="font-size: 9rem;"></td>
