    ...
    <v-client-table :columns="columns" :data="records" :options="table_options">
        ...
        <div slot="type" slot-scope="props" class="text-center">
            <div v-if="props.row.type == 'resource'">
                <span class="badge badge-success"><strong>Ingreso</strong></span>
            </div>
            <div v-else-if="props.row.type == 'egress'">
                <span class="badge badge-error"><strong>Egreso</strong></span>
            </div>
            <div v-else>
                <span class="badge badge-warning"><strong>No definido</strong></span>
            </div>
        </div>
...
export default {
  ...
  data() {
    return {
      records: [],
      columns: ['code', 'denomination', 'type', 'status', 'original', 'id']
    };
  },
  created() {
