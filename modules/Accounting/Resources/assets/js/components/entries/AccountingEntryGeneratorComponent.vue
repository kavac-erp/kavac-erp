        ...
        <table class="table table-formulation">
            <thead>
                <tr>
                    ...
                    <th class="text-uppercase" width="10%" v-if="showEdit == false">ACCIÓN</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(record, index) in recordsAccounting" :key="index">
                    <td>
                        <select2 :disabled="showEdit != false" :options="accounting_accounts" v-model="record.id" @input="changeSelectingTable(record)"></select2>
                    </td>
                    <td>
                        <input :disabled="record.assets != 0 || showEdit != false" type="number" class="form-control input-sm"
                               :step="cualculateLimitDecimal()" v-model="record.debit" @change="calculateTot()">
                    </td>
                    <td>
                        <input :disabled="record.debit != 0 || showEdit != false" type="number" class="form-control input-sm"
                               :step="cualculateLimitDecimal()" v-model="record.assets" @change="calculateTot()">
                    </td>
                    <td v-if="showEdit == false">
                        <div class="text-center">
                            <button @click="clearValues(recordsAccounting.indexOf(record))" class="btn btn-default btn-xs btn-icon btn-action"
                            ...
                            <button @click="deleteAccount(recordsAccounting.indexOf(record), record.entryAccountId)"
                                    class="btn btn-danger btn-xs btn-icon btn-action" title="Eliminar registro" data-toggle="tooltip" v-has-tooltip>
                ...
                <tr>
                    <td id="helpEntriesAccountSelect">
                        <select2 :options="accounting_accounts" id="select2" @input="addAccountingAccount()" v-if="showEdit == false"></select2>
                    </td>
                    <td id="helpEntriesTotDebit">
                        <div class="form-group text-center">Total Debe:
                            <h6>
                                <span>{{ data.currency.symbol }}</span>
                                <span v-if="parseFloat(data.totDebit).toFixed(data.currency.decimal_places) == parseFloat(data.totAssets).toFixed(data.currency.decimal_places) &&
                                            parseFloat(data.totDebit).toFixed(data.currency.decimal_places) >= 0" style="color:#18ce0f;">
                    ...
                    <td id="helpEntriesTotAssets">
                        <div class="form-group text-center">Total Haber:
                            <h6>
                                <span>{{ data.currency.symbol }}</span>
                                <span v-if="parseFloat(data.totDebit).toFixed(data.currency.decimal_places) == parseFloat(data.totAssets).toFixed(data.currency.decimal_places) &&
                                            parseFloat(data.totAssets).toFixed(data.currency.decimal_places) >= 0" style="color:#18ce0f;">
    ...
    methods: {
        ...
        calculateTot: function() {
            ...
            for (var i = recordAcc.length - 1; i >= 0; i--) {
                if (recordAcc[i].id != '') {
                    var debit = (recordAcc[i].debit != '' && typeof(recordAcc[i].debit) != 'undefined') ? recordAcc[i].debit : 0;
                    var assets = (recordAcc[i].assets != '' && typeof(recordAcc[i].assets) != 'undefined') ? recordAcc[i].assets : 0;

                    recordAcc[i].debit = parseFloat(debit).toFixed(this.data.currency.decimal_places);
                    recordAcc[i].assets = parseFloat(assets).toFixed(this.data.currency.decimal_places);
                    ...
                    this.data.totDebit = (recordAcc[i].debit != '' && typeof(recordAcc[i].debit) != 'undefined')
                        ? parseFloat(parseFloat(this.data.totDebit) + parseFloat(recordAcc[i].debit)).toFixed(this.data.currency.decimal_places)
                        : 0;
                    this.data.totAssets = (recordAcc[i].assets != '' && typeof(recordAcc[i].assets) != 'undefined')
                        ? parseFloat(parseFloat(this.data.totAssets) + parseFloat(recordAcc[i].assets)).toFixed(this.data.currency.decimal_places)
                        : 0;
                }
            }
        },
        ...
        chargeAccounts(recordToConverter) {
            ...
            if (recordToConverter) {
                axios.post(`${window.app_url}/accounting/entries/converterToEntry`, {
                    ...
                    vm.loading = true;
                    vm.accounting_accounts = response.data.accountingAccounts;
                    ...
                    function addRecordsAccounting () {
                        vm.recordsAccounting = Object.values(response.data.recordsAccounting).map(function ($acc) {
                            return {
                                'amount': ($acc.amount) ? parseFloat($acc.amount).toFixed(2) : 0,
                                'assets': ($acc.assets) ? parseFloat($acc.assets).toFixed(2) : 0,
                                'debit':( $acc.debit) ? parseFloat($acc.debit).toFixed(2) : 0,
                                'id': $acc.id,
