<template>
	<section id="AssetAsignationForm">
		<div class="card-body">
			...
			<div class="row">
				...
				<div class="col-md-4" id="authorized_by">
					<div class="form-group is-required">
						...
						v-model="record.authorized_by_id"></select2>
					</div>
				</div>
				<div class="col-md-4" id="formed_by">
					<div class="form-group is-required">
						...
						<select2 :options="payroll_staffs"
						v-model="record.formed_by_id"></select2>
					</div>
				</div>
				<div class="col-md-4" id="delivered_by">
					<div class="form-group is-required">
						...
						<select2 :options="payroll_staffs"
						v-model="record.delivered_by_id"></select2>
					</div>
				</div>
			</div>
			<br>
			...
			<hr>
			  <v-server-table :url="route_asset"
				@row-click="toggleActive" :columns="columns" :options="table_options" ref="tableResults">
...
<script>
	export default {
		...
		created() {
			...
			url += (vm.asignationid != null)
				? '/asignations/' + vm.asignationid
				: '/asignations';
		...
		mounted() {
			...
			if ((vm.asignationid) && (!vm.assetid)) {
				vm.loadForm(vm.asignationid);
            }
			else if((!vm.asignationid) && (vm.assetid)) {
				vm.selected.push(vm.assetid);
            }
