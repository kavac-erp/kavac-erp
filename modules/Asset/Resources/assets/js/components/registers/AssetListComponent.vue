<template>
  ...
  <v-server-table :url="route_list" :columns="columns" :options="table_options"
      ...
      <div class="d-inline-flex">
        <button @click.prevent="setDetails('AssetInfo', props.row.id, 'AssetInfo')"
                class="btn btn-info btn-xs btn-icon btn-action btn-tooltip"
                title="Ver registro" data-toggle="tooltip" data-placement="bottom" type="button">
          <i class="fa fa-eye"></i>
        </button>

        <button v-if="isAssigned(props.row)"
        ...
        <button v-if="isDisincorporable(props.row)"
        ...
        <button v-if="isEditable(props.row)"
...
export default {
  ...
  methods: {
    ...
    // De lo contrario, devuelve true (asignado o entregado).
      const assigned = row.asset_status.id == 10 &&
     ...
     */
     isDisincorporable(row){
      ...
      const disincorporated = row.asset_disincorporation_asset == null &&
    ...
    isEditable(row){
      ...
      const editable = row.asset_disincorporation_asset == null &&
     ...
     */
     isRemovable(row){
      const remove = row.asset_disincorporation_asset == null &&
