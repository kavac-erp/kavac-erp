...
Una vez instaladas las dependencias, ejecuta el comando a continuación para crear el archivo de configuración del entorno:
...
Esto generará un identificador único para la aplicación.
...
AUTH_VIEW | Establece la plantilla por defecto para el método de autenticación a utilizar

AUDIT_LIMIT | Indica el límite de registros a almacenar por cada modelo de la aplicación para la auditoria.

ACTIVE_DIRECTORY | Condición que establece el uso de Directorio Activo como método de autenticación

ACTIVE_DIRECTORY_URL | Establece la URL o IP del servidor del Directorio Activo

ACTIVE_DIRECTORY_DN | Establece los datos del Nombre Distinguido del Directorio Activo. Ej. CN=Nombre Apellido,OU=Users,DC=example,DC=com

ACTIVE_DIRECTORY_BASE_DN | Establece los datos del Nombre Distinguido Base del Directorio Activo. Ej. DC=example,DC=com
...
La aplicación dispone de una sección de registro o inicio de sesión donde se pide el ingreso de un captcha como certificación. El color del captcha puede variar, pero si se desea mantener sólo un color (negro) para mantener uniformidad, debe aplicar lo siguiente:
